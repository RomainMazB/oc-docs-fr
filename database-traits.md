...
> **Remarque** : Encrypted attributes will be serialized and unserialized as a part of the encryption / decryption process. Do not make an attribute that is `encryptable` also [`jsonable`](model#standard-properties) at the same time as the `jsonable` process will attempt to decode a value that has already been unserialized by the encryptor.
...
> **Remarque** : You're free to use the [array syntax](../services/validation#basic-usage) for validation rules as well.
...
> **Remarque** : You can also validate a model at any time using the `validate` method.
...
> **Remarque** : The Model leverages the MessagesBag object which has a [simple and elegant method](../services/validation#working-with-error-messages) of formatting errors.
...
> **Remarque** : If the related model does not use the soft delete trait, it will be treated the same as the `delete` option and deleted permanently.
