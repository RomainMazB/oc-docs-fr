...
> **Remarque** : Since HTML forms only support `POST` and `GET`, `PUT` and `DELETE` methods will be spoofed by automatically adding a `_method` hidden field to your form.
...
> **Remarque** : Most [data attributes from the AJAX framework](../ajax/attributes-api) are available here by dropping the `data-request-` prefix.
...
> **Remarque** : When using `Form::model`, be sure to close your form with `Form::close`!
...
> **Remarque** : After creating a label, any form element you create with a name matching the label name will automatically receive an ID matching the label name as well.
...
> **Remarque** : The *hidden* and *textarea* methods have the same signature as the *text* method.
...
> **Remarque** : The form must have been opened with the `files` option set to `true`.
...
> **Remarque** : Need to create a button element? Try the *button* method. It has the same signature as *submit*.
