...
## L'API JavaScript
L'API JavaScript est plus puissante que l'API des attributs de données.
La fonction `request` peut être utilisée sur n'importe quel élément à l'intérieur d'un formulaire, ou sur un formulaire.
Lorsque la fonction est utilisée sur un élément à l'intérieur d'un formulaire, elle est renvoyée au formulaire.
...
Le second argument de la fonction `request` est l'objet options. Vous pouvez utiliser toutes les options compatibles avec la [fonction AJAX de jQuery](http://api.jquery.com/jQuery.ajax/).
Les options suivantes sont spécifiques au framework d'October :
...
Exécute le gestionnaire `onCalculate` et s’il retourne un succès,
exécutes du code après que la méthode `success` par défaut ai terminée :
...
Ces évènements sont émis sur l'élément déclencheur :
