...
> **Remarque** : See the [installation guide](../setup/installation#crontab-setup) for instructions on how to set up the scheduler task.
...
> **Remarque** : The `emailOutputTo` and `sendOutputTo` methods are exclusive to the `command` method and are not supported for `call`.
