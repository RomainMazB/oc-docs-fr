...
> **Remarque** : Symfony HttpFoundation, which manages file downloads, requires the file being downloaded to have an ASCII file name.
...
> **Remarque** : Since the `with` method flashes data to the session, you may retrieve the data using the typical `Session::get` method.
...
> **Remarque** : The path hint is case sensitive and the plugin name should always be in lowercase.
